...
/* Who      When            What
   ---      ----            ----
   T. FUX   21 nov.20017    Add accessor setStatus(bool) and getStatus(bool) which permits to use QLed as a led only and not a led button
*/
...
class QLedIndicator : public QAbstractButton
{
    ...
    public:
        ...
        /* Modification done by Th. FUX 2017*/
        /* Allow to use QLed as a led onbly and not a led button */
        // Exemple of use: QLED.checkable(false); QLed.setStatus(true); // This turn the light ON
        bool on=false;
        void setStatus(bool b) { on = b; this->repaint();}
